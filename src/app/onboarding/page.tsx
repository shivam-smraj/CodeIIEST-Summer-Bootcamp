...
import {
  ...
  ExternalLink, AlertCircle, Shield, Trophy, Calendar, Star, ArrowRight,
} from 'lucide-react';
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    if (p.get('cf_success') === 'true') {
      ...
      setCfHandle(h); setCfRating(r); setCfStatus('success'); setStep(2);
  ...
  const onProfileSubmit = async (data: ProfileFormValues) => {
    ...
    try {
      ...
      toast.success('Profile saved!'); setStep(2);
  ...
  // Top accent bar color per step
  const accentBar =
    step === 1 ? 'linear-gradient(90deg,#dc2626,#f97316)' :
    step === 2 ? (cfStatus === 'success' ? 'linear-gradient(90deg,#16a34a,#22c55e)' : 'linear-gradient(90deg,#2563eb,#7c3aed)') :
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 480, display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, marginBottom: 20 }}>
            ...
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontSize: 9, fontWeight: 800, color: '#ef4444', textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: 1 }}>
                IIEST Shibpur
              </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {STEPS.map(({ id, label, icon: Icon }, i) => {
            ...
            const bdr    = done ? 'rgba(34,197,94,0.35)' : active ? 'rgba(220,38,38,0.40)' : 'rgba(255,255,255,0.08)';
            return (
              <div key={id} style={{ display: 'flex', alignItems: 'center' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                  <div style={{
                    ...
                    transition: 'all 0.3s ease', boxShadow: active ? `0 0 16px ${color}40` : 'none',
                  }}>
                    {done
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  ...
                  <div>
                    ...
                    <div style={{
                      ...
                      <span style={{ flex: 1, color: '#94a3b8', fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{email}</span>
                      <span style={{ fontSize: 10, fontWeight: 700, color: '#22c55e', background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.20)', borderRadius: 20, padding: '2px 9px' }}>
                        Verified
                      </span>
                    </div>
                  </div>
                  ...
                  {/* Roll ID parsed */}
                  {rollId && (
                    <div style={{ background: 'rgba(220,38,38,0.05)', border: '1px solid rgba(220,38,38,0.15)', borderRadius: 10, padding: '12px 14px' }}>
            ...
            {step === 2 && (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {/* Success */}
                  {cfStatus === 'success' && (
                    <div style={{ background: 'rgba(22,163,74,0.08)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: 14, padding: '20px 18px', textAlign: 'center' }}>
                      <div style={{
                      ...
                      {cfRating && (
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(250,204,21,0.08)', border: '1px solid rgba(250,204,21,0.20)', borderRadius: 20, padding: '4px 12px' }}>
                  ...
                  {/* Error */}
                  {cfStatus === 'error' && (
                    <div style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.20)', borderRadius: 12, padding: '14px 16px', display: 'flex', gap: 12 }}>
                  ...
                  {/* Steps */}
                  {cfStatus !== 'success' && (
                    <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {[
                        ...
                        <div key={n} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, background: 'rgba(37,99,235,0.12)', border: '1px solid rgba(37,99,235,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: '#60a5fa', fontFamily: 'monospace' }}>{n}</div>
            ...
            {step === 3 && (
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ color: '#64748b', fontSize: 14, lineHeight: 1.7, marginBottom: 24 }}>
                  ...
                  Sessions start on <span style={{ color: '#ef4444', fontWeight: 700 }}>June 1st, 2026</span>.
                </p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24, textAlign: 'left' }}>
                  {[
                    ...
                    <div key={label} style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '12px 14px' }}>
                ...
                <button onClick={handleFinish} disabled={submitting} style={{
                  ...
                  background: submitting ? 'rgba(220,38,38,0.4)' : 'linear-gradient(135deg,#dc2626 0%,#9333ea 100%)',
